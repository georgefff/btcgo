...
type MsgGetHeaders struct {
	...
	BlockLocatorHashes []*utils.Hash
	HashStop           utils.Hash
}

// AddBlockLocatorHash adds a new block locator hash to the message.
func (msg *MsgGetHeaders) AddBlockLocatorHash(hash *utils.Hash) error {
...
func (msg *MsgGetHeaders) BtcDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	locatorHashes := make([]utils.Hash, count)
	msg.BlockLocatorHashes = make([]*utils.Hash, 0, count)
...
func NewMsgGetHeaders() *MsgGetHeaders {
	return &MsgGetHeaders{
		BlockLocatorHashes: make([]*utils.Hash, 0,
			MaxBlockLocatorsPerMsg),
	}
}
